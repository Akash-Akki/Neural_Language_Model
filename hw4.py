...
Original file is located at
    https://colab.research.google.com/drive/1np1cneXaJpQuy9yLmVCFIlWXpCQl-zNK
...
def lowercase_corpus(original_corpus):
    ...
    for i in range(0,len(original_corpus)):
      list1=[]
      for j in range(0,len(original_corpus[i])):
            word = original_corpus[i][j]
            word = word.lower()
            list1.append(word)
      lower_case_corpus.append(list1)
    return lower_case_corpus
...
vocabulary = []
for key in new_word_counts.keys():
...
def convert_to_one_hot(word, vocabulary):
    one_hot_vector = torch.zeros(len(vocabulary),1)
    i= 0;
    if word in vocabulary:
       one_hot_vector[vocabulary[word]] =1;  
      ...
      for i  in range(0,len(corpus)):
        for j in range(0,len(corpus[i])):
          ...
          if word in vocabulary:
            ...
            l = k
            list_hot_vectors = []
            while l!=0 and index>=0:
              hotVector = convert_to_one_hot(corpus[i][index],vocabulary)
              list_hot_vectors.append(hotVector)
              l-=1
              index-=1
            while l!=0:
              hotVector = convert_to_one_hot('<s>',vocabulary)
              list_hot_vectors.append(hotVector)
              l-=1
            yield wordIndex,list_hot_vectors
...
def initialize_parameter(parameter_size):
    return (torch.randn(parameter_size,requires_grad=True)*(sqrt(2/(parameter_size[0]+parameter_size[1]))))
...
class NLM:
  ...
  def __init__(self, vocabulary_size, embedding_length=64, k=4):
    ...
    self.W1 = initialize_parameter((embedding_length*k,embedding_length*k))
    self.W2 = initialize_parameter((vocabulary_size,embedding_length*k))
    self.b1 = initialize_parameter((embedding_length*k,1))
  ...
  def forward_pass(self, context):
    eList=[]
    for c in context: 
        e = torch.matmul(self.E,c)
        eList.append(e)
    x = torch.cat(eList)
    p = torch.matmul(self.W1,x) 
    z=torch.add(p,self.b1)
    h=torch.tanh(z)
    p2= torch.matmul(self.W2,h)
    c = p2+self.b2
    return torch.nn.functional.log_softmax(c,dim=0)
...
def train(model, corpus, vocabulary, learning_rate=0.1):
  ...
  for epoch in range(0,n):
        for wordIndex, hot_vectors in generate_training_pairs(corpus, vocabulary):
          i = wordIndex
          hot_coding =hot_vectors
          y_hat = model.forward_pass(hot_coding)
          #cross entropy
          l = -y_hat[i]
          l.backward(retain_graph=True)
          model.E.retain_grad()
          model.W1.retain_grad()
          model.W2.retain_grad()
          model.b1.retain_grad()
          model.b2.retain_grad() 
          with torch.no_grad():
...
def evaluate_wordsim(word_pairs, gold_scores, embedding_matrix, vocabulary):
  ...
  for key in word_pairs:
    word_pair1 = vocabulary.get(key[0],-1)
    word_pair2= vocabulary.get(key[1],-1)
    if word_pair1 == -1:
       word_pair1= "<unk>"
    else:
       word_pair1 = key[0]
    if word_pair2 == -1:
       ...
       word_pair2 = key[1]
    embedding_word_pair1 = convert_to_one_hot(word_pair1,vocabulary).squeeze()
    embedding_word_pair2 = convert_to_one_hot(word_pair2,vocabulary).squeeze()
    e_word_pair1 = torch.matmul(embedding_matrix,embedding_word_pair1)
    e_word_pair2= torch.matmul(embedding_matrix,embedding_word_pair2)
    cosine_simlarity = get_cosine_similarity(e_word_pair1,e_word_pair2)
    classified_values.append(cosine_simlarity)
  result =spearmanr(gold_scores,classified_values)
  return result
